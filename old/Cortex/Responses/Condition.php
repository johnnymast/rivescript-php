 ...
 *
 * The ConditionCommand class determines the type of condition and
 * executes it.
 *
...
class Condition extends Response implements ResponseContract
{
    ...
    /**
     * Handle ConditionCommand, if the source is a ConditionCommand
    ...
    public function parse()
    {
        if ($this->responseQueueItem()->getCommand() === '*') {
            foreach (synapse()->conditions as $class) {
                $class = "\\Axiom\\Rivescript\\Cortex\\ConditionCommand\\{$class}";
                $instance = new $class();
