 ...
 * Get class
 *
 * This class is responsible parsing the <get> tag.
 *
...
class __Get extends Tag
{
     ...
     * @var string
     */
    protected string $pattern = "/<get (.+?)>/u";
    ...
    public function parse(string $source, SourceInput $input): string
    {
        ...
        while ($this->hasMatches($source)) {
            ...
            foreach ($matches as $match) {
                $userData = synapse()->memory->user($input->user())->get($match[1]) ?? "undefined";
                $source = str_replace($match[0], $userData, $source);
            }
        }
