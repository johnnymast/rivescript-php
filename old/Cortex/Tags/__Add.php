...
class __Add extends Tag
{
     ...
     * @var string
     */
    protected string $pattern = "/<add (.+?)=(.+?)\B>|<add (.+?)=(.+?)>/u";
        ...
        if ($this->hasMatches($source)) {
            ...
            foreach ($matches as $match) {
                ...
                if (is_numeric($value) === false) {
                    return "[ERR: Math can't 'add' non-numeric value '{$value}']{$source}";
                } elseif ($variable === "undefined" || is_numeric($variable) === false) {
                    return "[ERR: Math can't 'add' non-numeric user variable '{$name}']{$source}";
                }
