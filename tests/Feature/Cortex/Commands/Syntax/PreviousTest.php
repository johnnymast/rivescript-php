...
it('rejects invalid characters', function () {
    ...
    $expected = "PreviousCommand may only contain lowercase letters, numbers, and these symbols: ( | ) [ ] * _ # @ { } < > =";
...
it('rejects invalid characters for UTF8 mode', closure: function () {
    ...
    $expected = "PreviousCommand can't contain uppercase letters, backslashes or dots in UTF-8 mode.";
    $errors = $command->getSyntaxErrors();
    $actual = current($errors);
